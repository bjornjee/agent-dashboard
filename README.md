...
Both interfaces read agent state from per-agent JSON files in `~/.agent-dashboard/agents/` (written by the Claude Code adapter in `adapters/claude/`).
...
├── adapters/claude/              # Claude Code plugin (see below)
└── schema/
