...
echo ""
echo "  4. (Codex users only) Register the same plugin with codex so state"
echo "     files appear for codex-harness sessions. Codex's hook schema is"
echo "     1:1 with Claude's, so the existing adapter works unchanged:"
echo ""
echo "     codex plugin marketplace add bjornjee/agent-dashboard"
echo ""
echo "     Then add the following to ~/.codex/config.toml and restart codex:"
echo ""
