...
**Delegation gate:** Run `codex --version`. If Codex CLI is available, invoke `/codex-delegate` with the diagnosis (Phase 4) and failing test (Phase 3) as implementation context, then skip to the phase gate. If unavailable or user opts out, proceed below.
