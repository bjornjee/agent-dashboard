...
const HOOK_PATH = path.join(__dirname, 'warn-destructive.js');
...
describe('hook integration: stdout contract', () => {
  ...
  it('emits "{}\\n" on stdout for non-blocking input', () => {
  ...
  it('blocks git reset --hard with exit 2 and stderr reason', () => {
    const result = runHook({
      session_id: 'test-2',
      hook_event_name: 'PreToolUse',
      tool_name: 'Bash',
  ...
  it('allows git worktree remove --force (worktree exception)', () => {
    const result = runHook({
      session_id: 'test-3',
      hook_event_name: 'PreToolUse',
      tool_name: 'Bash',
      tool_input: { command: 'git worktree remove --force foo' },
    });
    assert.equal(result.status, 0, `stderr=${result.stderr}`);
    assert.equal(result.stdout, '{}\n');
  });
