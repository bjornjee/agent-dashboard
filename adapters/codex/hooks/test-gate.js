#!/usr/bin/env node
/**
 * PostToolUse hook for Bash — fails git commit commands unless `make test` passes.
 *
 * Delegates entirely to the project's Makefile. If no Makefile or test target
 * exists, nudges the user to add one (but does not signal an error).
 *
 * Set SKIP_TEST_GATE=1 to bypass (WIP commits, docs-only changes).
 * Fires after the command completes; exit code 2 signals the error back to
 * the model so it can undo the commit. On pass-through emit `{}` so Codex's
