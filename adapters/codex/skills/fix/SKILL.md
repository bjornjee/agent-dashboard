...
5. Create branch `fix/<name>` and worktree `../worktrees/<app>/<name>` from main with separate `exec_command` tool calls:
   ...
   - Run `git worktree add -b fix/<name> ../worktrees/<app>/<name> main` as its own `exec_command` tool call. Do not combine it with `mkdir`, `cd`, `&&`, `;`, or pipes; the Codex dashboard hook observes this standalone command to pin the worktree directory and branch.
