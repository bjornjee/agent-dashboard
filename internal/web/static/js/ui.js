...
export const UI = {
  ...
  // `tag` (optional): small inline chip rendered after the title (e.g. "PR").
  ...
  row(opts) {
    ...
    return `<${el} class="ui-row"${click}>${lead}
      <div class="ui-row__body"><span class="ui-row__title-line"><span class="ui-row__title">${escapeHtml(o.title || '')}</span>${tagChip}${badges}</span>${sub}</div>
      ${trail}
    </${el}>`;
  },
