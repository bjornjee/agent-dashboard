...
- **Plan-required skills auto-enter plan mode at spawn.** Dashboard-created Codex sessions for plan-required skills (`feature`) launch without the CLI prompt positional — Codex would auto-submit it before plan mode is active. Instead, the dashboard waits for Codex's post-configure session banner (`model: <name>`, no longer `loading` — Codex resets the collaboration mode when its session finishes configuring, so a `/plan` injected earlier is silently lost), pastes `/plan` and retries until the "Plan mode" footer confirms, then pastes the skill prompt. Injection addresses the stable pane id and aborts if the pane disappears. If any gate times out, injection proceeds anyway and the skill's own "stop and ask the user to run `/plan`" gate is the fallback.
