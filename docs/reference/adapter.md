...
- **Plan-required skills auto-enter plan mode at spawn.** Dashboard-created Codex sessions for plan-required skills (`feature`) launch without the CLI prompt positional — Codex would auto-submit it before plan mode is active. Instead, the dashboard waits for the composer to render **and** for the pane's agent state file to appear (Codex resets the collaboration mode when its session finishes configuring, so a `/plan` injected earlier is silently lost; the SessionStart hook writes the state file only after that point). It then pastes `/plan`, verifies the "Plan mode" footer, and pastes the skill prompt. If any gate times out, injection proceeds anyway and the skill's own "stop and ask the user to run `/plan`" gate is the fallback.
