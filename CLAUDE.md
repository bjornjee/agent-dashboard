...
The dashboard supports three coding-agent harnesses today: `claude`, `pi`, and `codex`. Per-harness logic lives in `internal/harness/<name>/` for spawn-command construction and `internal/<harness>/` for reading the harness-specific state files. Adding a fourth harness means:
